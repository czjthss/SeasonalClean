...
class DomainEngine:
    ...
    def setup_attributes(self):
        ...
        logging.debug("DONE with pruned co-occurring statistics in %.2f secs", time.process_time() - tic)
    ...
    def generate_domain(self):
        ...
        for row in tqdm(list(records)):
            ...
            for attr in self.active_attributes:
                ...
                cells.append({"_tid_": tid,
                              ...
                              "_cid_": cid,
                              "_vid_": vid,
                              "domain": "|||".join([str(i) for i in dom]),
                              "domain_size": len(dom),
        ...
        logging.debug('DONE generating initial set of domain values in %.2f', time.process_time() - tic)
        ...
        logging.debug('DONE training posterior model in %.2fs', time.process_time() - tic)
        ...
        logging.debug('DONE predictions in %.2f secs, re-constructing cell domain...', time.process_time() - tic)
        ...
        for preds, row in tqdm(zip(preds_by_cell, domain_df.to_records())):
            ...
            row['domain'] = '|||'.join([str(i) for i in domain_values])
        ...
        logging.debug('DONE assembling cell domain table in %.2fs', time.process_time() - tic)
